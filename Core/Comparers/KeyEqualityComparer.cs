...
namespace DogmaMix.Core.Comparers
{
    ...
    public class KeyEqualityComparer<TSource, TKey> : EqualityComparer<TSource>
    {
        ...
        /// </summary>
        /// <param name="keySelector">A function to extract the key from a source item.</param>
        /// <param name="innerEqualityComparer">The equality comparison operation to apply to the extracted keys.</param>
        /// <exception cref="ArgumentNullException"><paramref name="keySelector"/> or <paramref name="innerEqualityComparer"/> is <see langword="null"/>.</exception>
        public KeyEqualityComparer(Func<TSource, TKey> keySelector, IEqualityComparer<TKey> innerEqualityComparer)
        {
            ArgumentValidate.NotNull(keySelector, nameof(keySelector));
            ArgumentValidate.NotNull(innerEqualityComparer, nameof(innerEqualityComparer));
